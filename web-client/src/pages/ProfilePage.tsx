...
const STATUS_PRIORITY: SaveStatus[] = [
	"error",
	"saving",
	"resaving",
	"saved",
...
export function ProfilePage() {
	...
	const [pendingDietDeletion, setPendingDietDeletion] = useState<number | null>(
		null,
	);
	const [pendingAllergyDeletion, setPendingAllergyDeletion] = useState<
		number | null
	>(null);
	...
	async function updateProfile(
		...
		if (res.status === 409)
			throw new Error(await errorMessage(res, t("profile.usernameTaken")));
		if (res.status === 400)
			throw new Error(await errorMessage(res, t("profile.invalidRequest")));
	...
	const {
		statuses: prefsStatuses,
		notifyEdit,
		trackSave,
	} = usePrefsAutosave<PrefsDraft>({
		save: savePrefs,
		onError: (err) => {
	...
	const livePrefs = (overrides: Partial<PrefsDraft> = {}): PrefsDraft => ({
		...
		allergies: arrayWithout(
			overrides.allergies ?? allergies,
			pendingAllergyDeletion,
		),
	});
	...
	async function deleteRow(list: "diet" | "allergies", index: number) {
		const setPending =
			list === "diet" ? setPendingDietDeletion : setPendingAllergyDeletion;
	...
	const dietPlaceholders = t("profile.dietPlaceholders", {
		returnObjects: true,
	});
	const allergyPlaceholders = t("profile.allergyPlaceholders", {
		returnObjects: true,
	});
			...
			<div className="w-full max-w-md rounded-lg border border-gray-200 bg-white p-6 shadow-sm self-center md:self-start">
				<div className="flex flex-col gap-4">
						...
						{allergies.map((allergy, i) => (
							...
							>
								<input
									...
									placeholder={
										allergyPlaceholders[i % allergyPlaceholders.length]
									}
								/>
								<button
									type="button"
					...
					{prefsStatus && (
						<p
							className={
								prefsStatus.kind === "error" ? "text-red-600" : "text-green-600"
							}
						>
							{prefsStatus.msg}
						</p>
